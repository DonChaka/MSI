...
class ANFIS:
    ...
    def __init__(self, inputs, training_data: np.ndarray, expected_labels: np.ndarray, s_norm: Callable,
                 t_norm: Callable, operator_init_value=0.5):
        ...
        self.norm_counter = len(tuple(product([self.t_norm, self.s_norm], repeat=2)))

        # self.tsk = np.ones((self.nodes_number ,self.input_number+1))
        self.tsk = np.random.random((self.nodes_number * self.norm_counter, self.input_number + 1))

        self.op = [operator_init_value] * self.nodes_number * self.norm_counter

        self.calculate_aids()

    def calculate_norms(self, args, op):
        temp = []
        for n1, n2 in product([self.t_norm, self.s_norm], repeat=2):
            temp.append(n2(np.array([n1(np.array([args[0], args[1]]), op), args[2]]), op))
        return hstack(temp)
    ...
    def anfis_estimate_labels(self, fv, op, tsk) -> np.ndarray:
        ...
        R = self.calculate_norms(arguments, self.op)
