...
class GeneticUnitFloat:
    ...
    def cross(self, other) -> tuple:
        ...
        cut_points = np.random.choice([i for i in range(self.n_features * self.feature_size)],
        ...
        return GeneticUnitFloat(self.cost_function, a, self.n_features, self.feature_size, self.n_cut_points,
                                self.n_mutate_points, self.cost_function_args), \
...
class GeneticEngine:
    ...
    def run(self, n_generations: int = 100, verbose: bool = True) -> None:
        ...
        for _ in pbar:
            ...
            title = f"Current best fitness: {float(-best_fitness):.6f}"
